...
This uses **Brevo's email API**, not Gmail SMTP. Render's free tier blocks
outbound SMTP traffic (the ports Gmail app passwords rely on), so this sends
mail over a normal HTTPS request instead, which isn't blocked. Brevo's free
plan covers 300 emails/day.

1. Sign up free at https://www.brevo.com
2. Go to **Settings → Senders, Domains & Dedicated IPs → Senders tab → Add a sender**
   - Enter the "from" address you want orders to appear to come from (any
     address you control — doesn't have to be `inkdcustomcreations@gmail.com`)
   - Brevo emails that address a confirmation link — click it to verify
...
```
BREVO_API_KEY=xkeysib-the-key-you-just-generated
EMAIL_USER=the-address-you-verified-in-step-2@gmail.com
ORDER_NOTIFICATION_EMAIL=inkdcustomcreations@gmail.com
```

`EMAIL_USER` is the verified sender — `ORDER_NOTIFICATION_EMAIL` is where
orders actually land, and does **not** need to be verified in Brevo. If you
skip `ORDER_NOTIFICATION_EMAIL`, orders go to `EMAIL_USER` instead, so set it
explicitly if those two should be different accounts.
